...
$order="Select * from porders join porder_item on porder_item.purid=porders.purid and porders.purid=$orderid and porder_item.purid=$orderid ";
// echo $order;
// die;
...
if(isset($_POST["create_invoice"]))
  {
   ...
   // 
    ...
    $sql_insert ="update `porders` set `sid`=$partyname, 
    `invno`='$invno', 
    ...
    `order_total_tax1`= '$order_total_tax1',
     ...
     `order_total_after_tax`='$order_total_after_tax' where purid='$orderid'";
    //  echo $sql_insert;
    //  die();
     $res_insert = mysqli_query($con, $sql_insert);
     $ds=mysqli_query($con,"Select  * from porder_item where purid=$orderid ");
     
     foreach($ds as $ks){
      $productqty=$ks['order_item_quantity'];
      $productidq=$ks['productid'];
      mysqli_query($con,"update products set stock=stock-'$productqty' where productid='$productidq'");
    }
    mysqli_query($con,"Delete from porder_item where purid='$orderid'");
    ...
    //  order_item_tax1_rate	order_item_tax1_amount	order_item_tax2_rate	order_item_tax2_amount	order_item_final_amount	
     ...
     for($i=0;$i<$service;$i++){   
       ...
       $sql_service = "insert into porder_item(purid,productid,order_item_quantity,order_item_price,order_item_actual_amount,order_item_tax1_rate,order_item_tax1_amount,order_item_tax2_rate,order_item_tax2_amount,order_item_final_amount	)
       values
      ...
      //  die();
       ...
       mysqli_query($con,"update products set stock=stock+'$order_item_quantity[$i]' where productid='$productname[$i]'");
     }
     echo '<script>alert("Records Updated")
          
     </script>'; 
     echo "<script> document.location.href='purchase.php';</script>"; 
          ...
          <table class="table table-bordered">
            ...
            <tr>
                <td colspan="2">
                  <div class="row">
                    ...
                    <?php
                       while ($party= mysqli_fetch_array($pdetails)) {
                        ...
                        <option value='<?php echo $party['sid'];?>'
                         <?php if($odrpartyid== $party['sid'])
                         {echo 'selected';} ?>> <?php echo $party['companyname'] ?></option> 
                       <?php
                       }
                  ...
                  <div class="form-group">
                          <label for="bday">Invoice No.</label>
                          <input type="text" name="invno" class="form-control" id="" placeholder="" value="<?php echo $invoice; ?>">
                        </div>    
                  ...
                  <tr>
                    ...
                    <?php
                    $item_result=mysqli_query($con,"Select * from porder_item where purid=$orderid");
                    $m = 0;
                    ...
                    <tr class="abc">
                      ...
                      <td><input type="text" name="order_item_actual_amount[]" id="order_item_actual_amount<?php echo $m; ?>" data-srno="<?php echo $m; ?>" class="form-control input-sm order_item_actual_amount" value="<?php echo $sub_row["order_item_actual_amount"];?>"  /></td>
                      ...
                      <td><button type="button" name="rmv" id="<?php echo $m; ?>" class="btn btn-danger btn-xs rmv">X</button></td>
                      <td></td>
                    </tr>
                    <?php
                    }
